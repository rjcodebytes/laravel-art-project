...
@section('content')
    <div class="p-6">
        ...
        <form action="{{ route('admin.myblog.update', $blog->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="editor" rows="10"
                    class="w-full border rounded-lg p-2">{{ old('description', $blog->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="keywords" value="{{ old('keywords', $blog->keywords) }}"
                    placeholder="e.g. art, ajanta, painting" class="mt-1 w-full border rounded-lg p-2">
            </div>


            <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded hover:bg-indigo-700">Update
                Blog</button>
        </form>
    </div>
@endsection